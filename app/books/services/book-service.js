...
function getNextPrevBookId(id) {
    return storageService.query(BOOK_KEY)
        .then(books => {
            ...
            if (prevIdx === -1) prevIdx = books.length - 1
            return {
...
function convertBook(book) {
    ...
    bookToSave.title = volumeInfo.title
    bookToSave.subtitle = volumeInfo.subtitle
    bookToSave.authors = volumeInfo.authors
    bookToSave.publishedDate = volumeInfo.publishedDate
    bookToSave.description = volumeInfo.description
    bookToSave.pageCount = volumeInfo.pageCount
    bookToSave.categories = volumeInfo.categories
    bookToSave.thumbnail = volumeInfo.imageLinks.thumbnail
    bookToSave.language = volumeInfo.language
    bookToSave.listPrice = {}
    bookToSave.listPrice.amount = 70
    bookToSave.listPrice.currencyCode = 'EUR'
    bookToSave.listPrice.isOnSale = false
...
function removeReview(reviewId, bookId) {
    return getById(bookId)
        .then((book) => { //use find index
            const updatedReviews = book.review.filter(review => {
...
function query() {
    return storageService.query(BOOK_KEY)
        .then(books => {
            ...
            if (!booksToReturn.length) {
                booksToReturn = gBooks
                booksToReturn.forEach((book) => {
                    book.review = []
                })
                storageService.postMany(BOOK_KEY, booksToReturn)
            }
